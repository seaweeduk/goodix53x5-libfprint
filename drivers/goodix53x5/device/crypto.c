  ...
  for (gsize i = 0; i + 1 < in_len; i += 2)
    {
      ...
      keystream_bits = (((((((
                ...
                (key & 0x2000)) >>
               2 |
              (adjacent_key_xor & 0x40) | (key & 0x20)) >>
             1 |
            ((key >> 9 ^ key << 8) & 0x800) |
            ((key >> 0x14 ^ key * 2) & 4) |
            ...
            ((key << 6 ^ key >> 7) & 0x100) |
            ((key & 0x100) << 7));
      keystream_bits = keystream_bits & 0xFFFFFFFF;
      key_low_word = key & 0xFFFF;
      key = ((key ^ (adjacent_key_xor >> 0x14 ^ key) >> 10) << 0x1F |
             key >> 1) &
            0xFFFFFFFF;

      ciphertext_word = (guint16) in[i] | ((guint16) in[i + 1] << 8);
      keystream_word = (guint16) (((keystream_bits >> 8) & 0xFFFF) +
                              ((keystream_bits & 0xFF) | (key_low_word & 1)) * 0x100);
      guint16 plaintext_word = ciphertext_word ^ keystream_word;
...
goodix_crypto_gtls_verify_identity (GoodixGtlsCtx *ctx)
{
  ...
  goodix_crypto_hmac_sha256 (ctx->hmac_key, 32, random_data, 64,
                             ctx->client_identity);

...
The configuration-repair predicate consumes the **final byte returned by the
first `ChangeMode` invocation**, including its second transport attempt when
the first returned zero. All bytes with low two bits set qualify, not only
literal 3. The subsequent mode-4 result and repeated `ChangeMode` result are
both ignored; there is no second configuration-repair test. Each repeated
`ChangeMode` invocation still owns its own one-zero-result retry, so a repair
sequence can issue up to four arm transactions in total. The wrapper then
stores wait state `0xf0` (down) or `0xf1` (up) and returns zero even if
configuration download and repeated arming both fail.

Unlike the identical transport retry inside one `ChangeMode`, the post-config
arm constructs a fresh local payload from the retained down/up base at
`0x180060730`/`0x180060748`. Parser updates received during the earlier arm or
configuration transaction can therefore affect this new payload. The wrapper
does not hold `0x180063870` across the configuration call: each `ChangeMode`
owns that lock separately; its ordinary HAL-action caller retains the outer
action serialization. Mode 4 rebuilds configuration from retained profile and
calibration through `Milan_DlCfg`, not from a modified arm packet. See
`usbinterface-FUN_180005094.md` for producer constraints and download ownership.
...
For multi-cell sends, the calls at `0x180018bc2` and `0x180018c4e` discard
intermediate `USBSend` results; only the final-cell result at `0x180018c9c`
controls entry to ACK polling. The fixed down/up/manual requests and sleep
fit one cell. Configuration download uses multiple cells, so its result must
not be interpreted as an independent check of every intermediate write.
`USBSend` accepts an unsigned-byte transfer length and holds protocol critical
section `0x180063848` while copying to its retained transfer buffer and calling
the WDF synchronous-write slot `+0x6e8`. Request and send-options arguments
are both null; no explicit write timeout is supplied at this call. Its only
status classification is signed negativity, not a per-error retry taxonomy.
The caller's ACK budget begins after this synchronous write returns and is
not a combined write-plus-ACK budget.

### Profile-9 Caller Budgets

The down/up arm wrapper `0x180005a60` passes 500 for each original and
post-configuration `ChangeMode` invocation; these branches have no response
wait. Manual acquisition `0x180005420` uses ACK 500 and response 500.
Configuration producer `0x180005094` calls `Dlcfg` (`0x18001aed8`) with
response budget 500; `Dlcfg` independently fixes ACK budget 500. EC control
uses its caller's 200 ACK budget and no response wait.

Mode-2 sleep receives a 16-bit caller timeout at argument `+4`. The concrete
mode-2 producers `OnActivate` (`0x1800214d0`), `deviceInit`
(`0x180020970`, including its sensor-check and initialization failure exits),
and `usbEvtDeviceD0Exit` (`0x180022ff0`) each write 200. They do not supply a
data-response timeout. Configuration mode 4 ignores this mode-argument
timeout and uses the configuration helper's own budgets.

These numbers are nominal milliseconds, implemented as polling counts:
ACK waiting increments once per `Sleep(1)`, while response waiting calls
`WaitForSingleObject(..., 50)` and adds 50 only after `WAIT_TIMEOUT`.
Thus a 500 response budget permits ten timeout waits. Scheduling and lock
acquisition can make elapsed wall time longer. Every transport retry starts
fresh counters. The independently running USB reader has no per-command or
per-continuation-cell deadline; partial framing does not restart the sender's
ACK or response counter. Sender completion depends on the complete packet's
ACK-slot/event publication, not on arrival of its first USB cell.

`SendDataToDeviceEx` (`0x180018dd8`) also distinguishes protocol lifetime
from a write result: it requires initialized byte `0x180063840 == 1`, and
does not call the sender when shutdown byte `0x18006a110` is nonzero after
acquiring its lock. Response-timeout polling checks that shutdown byte before
continuing. The outer zero-result retry does not bypass those lifetime checks
or reopen a stopped transport.
...
without a response copy or signal for command 7. An EC data reply received
